use leptos::{tachys::{renderer::dom::Dom, view::any_view::AnyView}, IntoView};
...
pub fn generate_app_function<IV>(app_fn: impl Fn() -> IV + Clone + Send + 'static) -> AppFunction where IV: IntoView + 'static {
...
// Provide a constructor for AppFunction so Pavex can generate it at will
// You'll need to put this inside another function so you can specify the function and not Pavex
pub fn generate_route_app_function<IV>(app_fn: impl Fn() -> IV + Clone + Send + 'static) -> RouteAppFunction where IV: IntoView + 'static {
    let any_view = app_fn().into_any();
