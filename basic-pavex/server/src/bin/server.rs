...
async fn main() -> anyhow::Result<()> {
    let subscriber = get_subscriber("basic_pavex".into(), "info".into(), std::io::stdout);
...
async fn _main() -> anyhow::Result<()> {
    ...
    let config = Config::load(None)?;let application_state = build_application_state(config.app).await;let tcp_listener = config
        .server
        .listener()
        .await
